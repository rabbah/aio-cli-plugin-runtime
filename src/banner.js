 ...
 */
ActivationBanner.makeBanner = (logFunc, activation, activationLogs) => {
  const end = moment(activation.end).format('MM/DD HH:mm:ss')
  const msg = chalk.dim('=== ') + chalk.bold(
...
ActivationBanner.bannerFunc = (logger) => {
  return (activation, activationLogs) => ActivationBanner.makeBanner(logger, activation, activationLogs)
}
