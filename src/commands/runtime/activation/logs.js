...
const { bannerFunc } = require('../../../banner')
...
class ActivationLogs extends RuntimeBaseCommand {
  async run () {
    ...
    if (!args.activationId) {
      ...
      const customLogger = { logFunc: this.log, bannerFunc: bannerFunc(this.log) }
      await rtLib.printActionLogs({ ow: owOptions }, customLogger, limit, filterActions, flags.strip, flags.poll || flags.tail || flags.watch)
    } else {
